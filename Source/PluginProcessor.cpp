...
void WaveshaperAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    if (!masterBypassBool) {
        ...
        {
            for (auto j = 0; j < buffer.getNumSamples(); ++j) //Process every sample of buffer
            {
                auto sample = buffer.getWritePointer(channel); //Get sample to process

                //Input Gain
                sample[j] *= Decibels::decibelsToGain(inputGainValue);

                //Waveshape & Wavefold process
                sample[j] = wavefoldProcess(sample[j]);

                //Output Gain
                sample[j] *= Decibels::decibelsToGain(outputGainValue);
            }
        }
...
float WaveshaperAudioProcessor::wavefoldProcess(float sampleToProcess)
{
    ...
    float waveshapeChoice = *valueTree.getRawParameterValue(CHOICE_ID);
    float tanhMultValue = *valueTree.getRawParameterValue(TANHMULT_ID);
    float sinFreqValue = *valueTree.getRawParameterValue(SINFREQ_ID);
    float sinAmountFloat = *valueTree.getRawParameterValue(SINAMOUNT_ID) * 0.005f;
    float twoPi = piValue * 2;
    float memorySample = sampleToProcess;

    //Tanh function applied to the signal
    sampleToProcess = std::tanh(sampleToProcess * tanhMultValue);

    switch((int)waveshapeChoice)
    {
    case -1: //Triangle (not really a pure triangle)
        sampleToProcess += sinAmountFloat * std::asin(std::sin(memorySample * (sinFreqValue-4)));
        sampleToProcess += sinAmountFloat * std::sin(memorySample * sinFreqValue*2);
        break;
    case 0: //Sinewave
        sampleToProcess += sinAmountFloat * std::sin(memorySample * sinFreqValue);
        break;
    }
...
float WaveshaperAudioProcessor::GaussianSmooth(float x) //not in use
{
    float exp = std::pow(x, 2) / 0.1f;
    float next = 1 / (0.5f*std::sqrt(2*piValue));
    float gaussianSmooth = std::pow(next, exp);
    return gaussianSmooth;
}

void WaveshaperAudioProcessor::MultiFilter(dsp::AudioBlock<float> bufferBlock)
{
    //Get parameters from GUI
...
void WaveshaperAudioProcessor::LowPassFilter(dsp::AudioBlock<float> bufferBlock)
{
    //Get parameters from GUI
